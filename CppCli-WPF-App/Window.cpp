...
*  Package Operations:
*  -------------------
*  This is the client 1 implementation of Pr4. Url=localhost:8080
*  
*  Required Files:
*  ---------------
*  Window.h, msclr\marshal_cppstd.h
*  Window.h, chrono
*
*  Build Command:
*  --------------
...
WPFCppCliDemo::WPFCppCliDemo(){
	...
	pclient_ = pObjFact->createclient(8080);
	...
	this->Title = "Client1(localhost:8080)"; this->Width = 800; this->Height = 600;
...
void WPFCppCliDemo::UploadFile(Object^ obj, RoutedEventArgs^ args)
{
	...
	Msg msg; msg.srcIP("127.0.0.1"); msg.srcPort(8080); msg.command("UpLoadFile");
	...
	if (connectserver1->IsChecked){
		...
		HWND hWnd = ::FindWindow(NULL, L"Message");
		if (hWnd){
